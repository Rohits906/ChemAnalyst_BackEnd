...
from .serializers import (
    ...
    TopLiveDataSerializer,
    RecentProfilePostsSerializer,
    FetchTaskSerializer,
    ChannelInfoSerializer,
    ChannelStatsSummarySerializer,
    ChannelBarDataSerializer,
    ChannelRecentPostsSerializer,
    ChannelTopPostsSerializer,
)
...
from .models import Platform as AddPlatform
...
class PlatformListCreateView(APIView):
    ...
    def get(self, request):
        try:
            platforms = AddPlatform.objects.filter(user=request.user).order_by(
                "-created_at"
            )
            data = []
            ...
            return Response(
                {"message": "Platforms fetched successfully", "data": data},
...
class PlatformDeleteView(APIView):
    ...
    def delete(self, request, pk):
        try:
            ...
            platform.delete()
            return Response(
                {"message": "Platform deleted successfully"}, status=status.HTTP_200_OK
            )
...
class PlatformCreateView(APIView):
    ...
    def post(self, request):
        ...
        existing = Platform.objects.filter(
            user=request.user, name=data["name"], channel_id=data["channel_id"]
        ).first()
...
class PlatformListView(APIView):
    ...
    def get(self, request):
        platforms = Platform.objects.filter(user=request.user, is_active=True).order_by(
            "-created_at"
        )
...
class PlatformDashboardView(APIView):
    ...
    def get(self, request):
        ...
        from_date = request.query_params.get("from") or request.query_params.get(
            "start_date"
        )
        ...
        response_data = {
            ...
            "topFive": TopLiveDataSerializer({"platforms": platforms, "limit": 5}).data,
    ...
    def post(self, request, platform_id=None):
        if platform_id:
            ...
            platform = get_object_or_404(
                Platform, id=platform_id, user=request.user, is_active=True
            )

            queue_platform_fetch(platform.id, "update")
            return Response({"message": f"Refresh triggered for {platform.name}"})
        else:
            # Refresh all platforms
            platforms = Platform.objects.filter(user=request.user, is_active=True)
...
class PlatformChannelDataView(APIView):
    ...
    def get(self, request, platform_name, channel_id):
        ...
        posts = platform.posts.filter(
            published_at__gte=period_start, published_at__lte=period_end
        )
...
class PlatformFetchTasksView(APIView):
    ...
    def get(self, request):
        tasks = PlatformFetchTask.objects.filter(user=request.user).order_by(
            "-created_at"
        )[:20]
...
class SentimentSearchTriggerView(APIView):
    ...
    def post(self, request, platform_id):
        platform = get_object_or_404(Platform, id=platform_id, user=request.user)

        # Get posts without sentiment
        posts = platform.posts.filter(sentiment_label__isnull=True)[:50]

        if not posts.exists():
            return Response({"message": "No posts to analyze"})
